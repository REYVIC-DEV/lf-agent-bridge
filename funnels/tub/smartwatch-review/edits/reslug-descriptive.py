...
# Descriptive slug, taken from the page's OWN breadcrumb ("Home > Wearables >
# Best Fitness Trackers of 2026") rather than invented, so the URL, the
# breadcrumb and the <title> all say the same thing. Reads as a real article in
# an ad's display URL, which an opaque id does not.
new_slug = "best-fitness-trackers-2026"
assert new_slug not in taken, "slug already used: %r" % new_slug
assert new_slug == new_slug.lower().strip("-"), "keep it lowercase, no edge dashes"
