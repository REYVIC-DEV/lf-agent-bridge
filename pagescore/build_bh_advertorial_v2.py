#!/usr/bin/env python3
"""New "TUB- BH Adventorial" funnel (fun_ZDkBMKGJqa6LE7150tkr0), step V2:
"The £79 Screenless Band That's Outselling Smartwatches in the UK 3x Over" --
a same-template headline-copy variant of V1 (build_bh_advertorial_v1.py),
source Figma file "HLTH" node 217:255 ("Branded Hype Advertorial V2",
desktop 1440 wide, no separate mobile companion frame exists for V2).

Confirmed via figwright scan_text_nodes on 217:255 (all 34 text nodes read
verbatim, not guessed): V2 is word-for-word IDENTICAL to V1 in every
...
screenshot-compared node-by-node against V2's Figma frame and are the SAME
assets as V1 (reused from bh_advertorial_img_map.json, no new upload).
...
V2-Mobile Figma frame exists to re-measure against, and the template/
...
longer V2 headline just wraps to more lines at both sizes (Title blocks
are HUG-height, no fixed-height style to adjust).

Real photos (byline avatar, section 1/3/5 images, 4 footer social icons)
are the SAME assets as V1 (screenshot-confirmed against 217:255's image
frames) -- reused from the existing pagescore/.cache/bh_advertorial_img_map.json,
no new upload needed.

Creates a second, detached step (slug bh-advertorial-v2) alongside the
existing V1 step via createStep in the "TUB- BH Adventorial" funnel, then
attaches it via updateFunnel. Does NOT touch starting_step_id (V1 stays
the funnel's entry step). Safe to re-run: if the slug already exists, it
redeploys that step's body in place instead of duplicating.
"""
...
kids=[]
kids.append(title("The £79 Screenless Band That's Outselling Smartwatches in the UK 3x Over",
...
TITLE = "The £79 Screenless Band That's Outselling Smartwatches in the UK 3x Over"
OG = json.load(open("pagescore/.cache/og_img_map.json"))  # QA: purpose-cut 1200x630 share images
SEO = {
    "title": "The £79 Screenless Band Outselling Smartwatches 3x Over",
    "description": "At just £79, the HLTH Band is outselling premium smartwatches 3x over in the UK. See why shoppers are switching to a screenless tracker that costs less and lasts longer.",
    "keywords": "HLTH Band price, £79 health tracker, cheap smartwatch alternative, affordable health tracker UK, screenless band vs smartwatch, best value fitness tracker 2026, budget health tracker",
    "social_image_uid": OG["bh-v2"]["src_uid"],
}
...
CUSTOM_HTML = {"header": PERF + (
    ...
    ) % (_esc(SEO["title"]), _esc(SEO["description"]), OG["bh-v2"]["src"]),
    "footer": PERF_FOOTER}
