#!/usr/bin/env python3
"""New "TUB- BH Adventorial" funnel (fun_ZDkBMKGJqa6LE7150tkr0), step V3:
"I Ditched My Smartwatch for a Screenless Band. Here's Why I'm Not Going
Back." -- another same-template headline-copy variant of V1
(build_bh_advertorial_v1.py), source Figma file "HLTH" node 217:114
("Branded Hype Advertorial V3", desktop 1440 wide, no separate mobile
companion frame exists for V3).

Confirmed via figwright scan_text_nodes on 217:114 (all 34 text nodes read
verbatim, not guessed): V3 is word-for-word IDENTICAL to V1/V2 in every
section -- byline, both intro paragraphs, all 5 numbered H2 sections,
blockquote, closing paragraphs, CTA button label, disclaimer, footer
links -- except the H1 headline, which is the only real copy change.
Section images (phone/wrist/teardown photos, byline avatar) were also
screenshot-compared node-by-node against V3's Figma frame and are the SAME
assets as V1/V2 (reused from bh_advertorial_img_map.json, no new upload).
Same template, same measured spacing/responsive rules as V1 below --
only the H1 content and page SEO differ.
...
Mobile responsive rules are carried over unchanged from V1 (no separate
V3-Mobile Figma frame exists to re-measure against, and the template/
layout is identical): same section order, same images, same body text
sizes throughout. H1 (40/52 -> 28/36), H2 (30/39 -> 22/30), and the byline
avatar (95px -> 64px circle) still shrink at the 767px breakpoint; the
new V3 headline just wraps to whatever line count it needs at both sizes
(Title blocks are HUG-height, no fixed-height style to adjust).

Real photos (byline avatar, section 1/3/5 images, 4 footer social icons)
are the SAME assets as V1/V2 (screenshot-confirmed against 217:114's image
frames) -- reused from the existing pagescore/.cache/bh_advertorial_img_map.json,
no new upload needed.

Creates a third, detached step (slug bh-advertorial-v3) alongside the
existing V1/V2 steps via createStep in the "TUB- BH Adventorial" funnel,
then attaches it via updateFunnel (upserts by id -- the other steps are
never resent, so they're left untouched). Does NOT touch starting_step_id
(V1 stays the funnel's entry step). Safe to re-run: if the slug already
...
kids=[]
kids.append(title("I Ditched My Smartwatch for a Screenless Band. Here's Why I'm Not Going Back.",
...
TITLE = "I Ditched My Smartwatch for a Screenless Band. Here's Why I'm Not Going Back."
OG = json.load(open("pagescore/.cache/og_img_map.json"))  # QA: purpose-cut 1200x630 share images
SEO = {
    "title": "I Ditched My Smartwatch for a Screenless Band",
    "description": "After 4 years of smartwatches, I switched to a screenless health tracker and I'm never going back. Here's what changed my mind about the HLTH Band.",
    "keywords": "smartwatch alternative, screenless health tracker review, HLTH Band review, why I switched from smartwatch, personal health tracker story, best wearable without screen, smartwatch vs health band",
    "social_image_uid": OG["bh-v3"]["src_uid"],
}
...
CUSTOM_HTML = {"header": PERF + (
    ...
    ) % (_esc(SEO["title"]), _esc(SEO["description"]), OG["bh-v3"]["src"]),
    "footer": PERF_FOOTER}
